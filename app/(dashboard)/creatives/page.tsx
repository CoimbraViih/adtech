...
export default async function CreativesPage({
  ...
  const [{ data: creativesData }, { data: campaignsData }] = await Promise.all([
    supabase.from("creatives").select("*").eq("workspace_id", session.workspace.id).order("created_at", { ascending: false }),
    supabase.from("campaigns").select("id, name").eq("workspace_id", session.workspace.id),
  ]);

  const allCreatives = creativesData ?? [];
  const campaignMap = Object.fromEntries((campaignsData ?? []).map((c) => [c.id, c.name]));
